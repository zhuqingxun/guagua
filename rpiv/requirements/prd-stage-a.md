...
### 4.2 整机 vs 散件（M2 决策门时再选）

| 选项 | 价格 | 优势 | 劣势 |
|------|------|------|------|
| **散件版（含技术支持）** | ¥3900 | 学习目标耦合最强（亲手摸 12 DOF 组装、URDF 对应物理实体）| 组装需 10-20 小时（占 M3 大部分） |
| 整机版（12V 强劲动力标准版）| ¥4199 | 省 1-2 周、开箱直接到 sim2real 阶段 | 失去 ncnynl 教程"硬件组装"章节的实战价值 |

**默认推荐：散件版**，与"彻底掌握技术栈"驱动力一致；用户在 M2 决策门时可改选整机版。
...
**4 项全过 → 进 M3，下单爱折腾**（散件 ¥3900 默认 / 整机 ¥4199 备选）。
**任一项不过 → 触发 M2 降级**（见 §7.2）。

### M3（第 7-8 周，20 小时）：硬件到货 + 组装 + 测试
**前提**：M2 决策门通过，硬件已下单（爱折腾发货 48 小时 + 顺丰 2-3 天 = 一周内到）。

- 散件版：按 ncnynl 27 篇前 5 章组装（硬件介绍 / 舵机配置 / 电路焊接 / 硬件组装 / 系统安装），10-15 小时
- 整机版：开箱跑爱折腾官方 demo（1-2 小时），剩余时间用于读 Runtime 源码
- 树莓派 OS Lite 64-bit 烧录 + SSH + I2C
- ONNX Runtime CPU 装好
- **本阶段交付**：硬件本体可控（舵机能动、IMU 能读、SSH 能进）
...
| 硬件选型 | 自购 BOM 7100 元 + 打印机 2000 元 | 爱折腾散件 3900 元（含技术支持）| 比自购便宜 3000 元，与 27 篇教程作者一致 |
| BOM 总额（阶段 A）| ~9000 元 | ~3900-4199 元 + 1000 元杂项 + 800 元云资源 = ~6000 元 | 节省 30%+ |
